  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600">Understanding your emotional patterns</p>
      ...
      {/* Affirmation Card */}
      <div className="bg-gradient-to-br from-emerald-400 to-teal-500 rounded-2xl p-6 text-white text-center">
        <Star className="w-8 h-8 mx-auto mb-3 opacity-80" />
        <p className="text-lg font-medium">{randomAffirmation}</p>
      </div>
      ...
      {/* Weekly Mood Tracker */}
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-6">This Week's Mood Journey</h3>
        <div className="flex justify-between items-end space-x-2">
          {weeklyData.map((day, index) => (
            <div key={index} className="flex-1 text-center">
              <div className="mb-3">
                {day.mood ? (
                  <div className={`w-12 h-12 mx-auto rounded-full bg-gradient-to-br ${day.mood.gradient} flex items-center justify-center text-xl border-2 border-white shadow-lg`}>
                    {day.mood.emoji}
                  </div>
                ) : (
      ...
      {moodStats && moodStats.moodCounts && Object.keys(moodStats.moodCounts).length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-6">Mood Breakdown</h3>
      ...
      {moodStats && moodStats.mostCommonMood && (
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 p-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-4">Your Most Common Mood</h3>
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{moodStats.mostCommonMood.label}</p>
      ...
      {moodEntries.length === 0 && journalEntries.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Start Building Insights</h3>
          <p className="text-gray-600 mb-6">
            Track your moods and write journal entries to see your emotional patterns bloom
          </p>
          <div className="flex justify-center space-x-4">
            <button className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-6 py-3 rounded-xl font-medium hover:from-emerald-600 hover:to-teal-700 transition-all duration-200">
